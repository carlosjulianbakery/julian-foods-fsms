      ...
      <div className="flex-1 relative" ref={ref}>
        ...
        {open && (
          <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-56 overflow-y-auto">
            ...
            {filtered.length === 0 ? (
              <div className="p-3 text-xs text-gray-400 text-center">No matched products found.</div>
            ) : (
              filtered.map((opt) => (
                <button
                  key={opt.id}
...
function BulkIgnoreBar({ count, onConfirm }: { count: number; onConfirm: () => void }) {
...
export function BundleConfigTab({ onUnmatchedCount }: { onUnmatchedCount?: (n: number) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    if (listRes.ok) {
      ...
      onUnmatchedCount?.(d.summary.unmatched);
    }
    if (presRes.ok) {
  ...
  function handleSaved(id: string, newStatus: ConfigStatus) {
    ...
    setSummary((prev) => {
      ...
      onUnmatchedCount?.(next.unmatched);
      return next;
    });
